...
describe('authService', () => {
  describe('Registeration method', () => {
    it('should handle errors during registration', async () => {
      ...
      try {
        await authService.Registeration(username, password, fullName);
      } catch (error) {
  ...
  describe('checkTokenExpiration method', () => {
    it('should correctly check if the token is expired', () => {
      const expirationTime = Math.floor(Date.now() / 1000) - 50; // Simulate an expired token
